...
const createChapterList = (options) => {
...
const createTitleElement = (name, num) => {
...
window.addEventListener("load", () => {
    if (window.location.hostname.includes("klz9.com")) {
        ...
        if (devMode) {
            ...
            window.addEventListener("keydown", ({ code, shiftKey }) => {
                ...
                if (shiftKey && code === "ArrowLeft")
                    window.selectBtns.prev?.click();
                if (code === "ArrowRight")
                    window.curReader?.nextPage();
                if (code === "ArrowLeft")
                    window.curReader?.prevPage();
                if (code === "KeyD") {
        ...
        const name = document.querySelector(".manga-name")?.innerText ??
            "No-title";
        const chapterNum = document.querySelector("#zlist-chs > select > option[selected]")?.innerText ?? "No-chapternum";
        const images = [
            ...document.querySelectorAll(".chapter-content p > img"),
        ];
